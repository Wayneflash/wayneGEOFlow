...
/**
 * URL 智能采集流水线提示词（官网直连 + AI 补充调研 + 清洗入库）。
 *
 * 设计原则：
...
RULE
            : '当前不调用实时搜索接口，可结合官网线索与模型公开认知汇总，但须在 evidence_limits 标注「未接入实时搜索，部分补充信息需人工核验」。';

        return <<<PROMPT
你是深联云 GEO 的「主体识别 + AI 补充调研」专家，服务于**企业 AI 知识库构建**与**后续 GEO 内容推广**。
...
请**以用户填写的公司名 + 品牌名 + 官网 URL 为轴心**做 AI 补充调研。当前不调用博查或实时搜索；官网标题、官网正文与用户填写信息是最高优先级证据，模型公开认知仅作补充。
...
SECTION;
        $liveSearchSection = $searchBlock !== ''
            ? $searchBlock."\n"
            : ($searchEnabled ? "【实时搜索】未返回可用条目，请在 evidence_limits 说明。\n" : "【实时搜索】未接入；当前为官网线索 + 模型公开认知的补充整理模式。\n");
...
page_json.text 可能来自：
- direct：官网直连
- hybrid：「【官网直连摘录】」+「【AI 补充调研汇总】」合并
- ai_research：主要靠 AI 补充调研
...
PROMPT;
    ...
    public static function cleanUser(array $pageJson): string
    {
        ...
        $modeNote = match ($mode) {
            ...
            'ai_research' => '素材主要来自 AI 补充调研。清洗时务必标注不确定性，禁止把推断当确定事实。',
...
## knowledge_markdown 结构（9 节，按顺序，禁止缺节；素材中无对应内容也要保留节并写「素材中未明确说明」）


### 1. 核心业务摘要
3-5 句，覆盖「主体 + 业务 + 客户 + 价值」四要素

### 2. 企业官网（**独立节，靠前位置便于 AI 引用**）
...
拿到上下文后，**先判断主体是「公司/品牌实体」还是「产品/概念大类」**，再决定标题形态：

A) **公司/品牌实体**（如「厦门磁北科技有限公司」「四信通信」这类完整法人主体）—— 厂商正向类标题必须是「实体介绍型」，**禁止**「X 是什么 / 为什么重要 / 怎么做 / 优势」这种概念类句式。模板如下：
   - `<公司名>：<业务/产品线/方案>介绍`
   - `<公司名>主营业务与解决方案`
   - `<公司名>在 <场景/行业> 的应用方案`
   - `<公司名> - <产品/方案>详情`
   - `<公司名>研发能力 / 资质 / 发展历程 / 行业经验`
   - `<公司名>联系方式与商务合作`
   - 例：「厦门磁北科技有限公司：商用车信息化业务介绍」「厦门磁北科技有限公司主营业务与解决方案」

B) **产品/概念大类**（如「智能客服」「工业物联网关」「AGV 调度系统」这类通用名词）—— 才允许用「是什么 / 为什么 / 怎么做 / 优势 / 原理 / 选型」句式。

判断依据：上下文里 `identified_company` 不为空、或 `title` 含「有限公司 / 股份 / Co., Ltd / Inc / LLC」字样 → 走 A 模板；否则走 B 模板。
...
1) **厂商正向 / 科普类**（≤60%）
   - A 实体型：见上方 A 模板，**不可自卖自夸**（不要「我们/自家/本公司推荐」）
   - B 概念型：是什么、为什么、怎么做、优势解读、原理、案例拆解
   - 例（A 实体型）：「厦门磁北科技有限公司：商用车信息化业务介绍」「厦门磁北主营业务与解决方案」
   - 例（B 概念型）：「工业物联网关选型要看的 5 个参数」
...
- 标题库必须混合两类：品牌锚点标题明确点名主体公司/品牌，用户决策标题保持中立选型口吻
- 若识别到主体公司/品牌，标题总量中至少 40% 点名主体公司/品牌，用于建立公司与产品词的关联
- 没有依据时不要写具体年份或「2026 趋势」类标题
- 标题中**不要重复完全相同的模板**；同一品牌名可以出现，但要围绕不同产品、场景、能力展开
- **实体型主体严禁出现「X 是什么 / 为什么 / 怎么做 / 优势解读 / 原理」这种概念类句式**
PROMPT;
    }
    ...
    public static function titlesUser(
        ...
        return "【输入来源】节点 ai_keywords 的 keywords + 节点 ai_knowledge 的 knowledge_markdown\n"
            ...
            .($pageJson['identified_company'] !== '' && ! empty($pageJson['identified_company'])
                ? "【特别提醒】本次识别的主体公司是「".$pageJson['identified_company']."」。标题需要兼顾品牌锚点与中立选型：至少 40% 标题点名该公司/品牌，其余可用「盘点」「这 X 家」「有哪些品牌」等中立表达。\n\n"
                : '')
            ."上下文：\n"
            .json_encode([
                'source_url' => $pageJson['source_url'] ?? '',
...
标题库：
- 驱动后续文章：产品/方案/案例介绍、决策榜单、行业对比、选型指南、FAQ
- **先判断主体形态**再选模板：
  - 「公司/品牌实体」→ 厂商正向类必须是「实体介绍型」：`<公司名>：<业务/方案>介绍` / `<公司名>主营业务与解决方案` / `<公司名>在 <场景/行业> 的应用方案` / `<公司名> - <产品/方案>详情` / `<公司名>研发能力 / 资质 / 发展历程` / `<公司名>联系方式与商务合作`；**严禁**「<公司名> 是什么 / 为什么 / 怎么做 / 优势 / 原理」概念类句式
  - 「产品/概念大类」→ 才允许「是什么 / 为什么 / 怎么做 / 优势 / 原理」句式
- **视角平衡**：
  - 厂商正向 / 科普类：≤60%，介绍产品能力、方案解读、技术原理、案例拆解
  - 用户决策 / 榜单推荐类：≥40%，覆盖「XX 哪家好」「XX 厂商推荐」「XX 公司排行」「XX 地区 XX 服务商」「XX 怎么选 / 避坑」等真实用户提问
- **品牌锚点**：若识别到公司/品牌，标题库至少 40% 明确点名主体公司/品牌；中立决策标题可不点名，用「盘点」「这 X 家」「有哪些品牌」等表达
...
1. **核心业务摘要**（3-5 句，覆盖「主体 + 业务 + 客户 + 价值」四要素）
2. **企业官网**（URL + 商城 + 自媒体矩阵 + 主要板块；**所有 GEO 文章都应自然带上官网链接**）
3. **公司基础信息**（全称 / 简称 / 英文名 / 性质 / 成立时间 / 总部地址 / 分支机构）
4. **主营业务**（一句话定位 / 业务范围 / 行业 / 商业模式）
5. **产品体系**（按业务线分组：产品名 + 型号 + 核心功能 + **技术亮点/差异化卖点** + 适用场景 + 技术参数 + 配套服务 + 价格区间）
6. **解决方案**（面向行业 / 方案组成 / 痛点 / 价值主张 / 典型配置；**写清方案亮点，不要只列产品名**）
7. **典型客户与案例**（客户类型 + 公开客户名单 + 项目案例 + 展会亮相 + 标杆项目）
8. **企业规模、资质与研发**（合并节：员工/营收/工厂 + 专利/认证/奖项 + 核心技术/自研芯片/合作 + 海外业务）
9. **联系方式**（电话/客服/邮箱（按角色）/地址/公众号/视频号/抖音号/招聘/投标）
...
- 若识别到主体公司/品牌，titles 至少 40% 明确点名主体公司/品牌；其余用户决策/榜单/选型标题保持中立口吻
- 禁止「第一/最好/领先」等无来源表述
- 每条标题应绑定至少 1 个 keywords 中的词或同义表达
PROMPT;
    }
...
## knowledge_markdown 结构（9 节，顺序固定，禁止缺节；素材中无对应内容也要保留节并写「素材中未明确说明」）
1. 核心业务摘要（3-5 句，覆盖主体 + 业务 + 客户 + 价值）
2. 企业官网（URL + 商城 + 自媒体矩阵 + 主要板块；**所有 GEO 文章都应自然带上官网链接**）
3. 公司基础信息（全称 / 简称 / 英文名 / 性质 / 成立时间 / 总部地址 / 分支机构）
4. 主营业务（一句话定位 / 业务范围 / 行业 / 商业模式）
5. 产品体系（按业务线分组：产品名 + 型号 + 核心功能 + **技术亮点/差异化卖点** + 适用场景 + 技术参数 + 配套服务 + 价格区间）
...
- **先判断主体形态**：若 `identified_company` 非空或 `title` 含「有限公司 / 股份 / Co., Ltd / Inc / LLC」→ 主体是「公司/品牌实体」。实体型主体的厂商正向标题必须是「实体介绍型」：
  - 模板：`<公司名>：<业务/产品线/方案>介绍` / `<公司名>主营业务与解决方案` / `<公司名>在 <场景/行业> 的应用方案` / `<公司名> - <产品/方案>详情` / `<公司名>研发能力 / 资质 / 发展历程 / 行业经验` / `<公司名>联系方式与商务合作`
  - **严禁**「<公司名> 是什么 / 为什么 / 怎么做 / 优势 / 原理 / 案例」这种概念类句式
- 若主体是「产品/概念大类」（智能客服 / 工业物联网关 / AGV 调度系统这类）才允许「是什么 / 为什么 / 怎么做 / 优势 / 原理」句式
- 若识别到主体公司/品牌，标题总量至少 40% 明确点名主体公司/品牌，形成公司名 + 产品词/场景词的关联
- 其余决策类可不点名主体公司，用「盘 XX 厂」「这几家」等中立表述
- 禁止「最好/第一/领先」等无来源表述；不要机械重复同一标题模板
PROMPT;
